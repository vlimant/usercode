...
#include "RecoMuon/TrackingTools/src/MuonErrorMatrixDump.h"
  ...
  MuonErrorMatrixAnalyzer::analyze_from_errormatrix(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (uint it=0;it!=tracks->size();++it){
    ...
    if (PCAstate.position().mag()==0)
     ...
     //prepare one entry per muon association
     theDump->clean();
     theDump->run = iEvent.id().run();
     theDump->event = iEvent.id().event();
     //fill the class
     theDump->fillReco(track_CTP_vector, track_error, point_track, momentum_track);
     //one entry per muon
     theTree->Fill();
     //the rest will not be very useful
...
MuonErrorMatrixAnalyzer::analyze_from_pull(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (reco::RecoToSimCollection::const_iterator RtSit=recSimColl.begin();RtSit!=recSimColl.end();++RtSit){
	...
	theDump->clean();
        ...
        theDump->event = iEvent.id().event();
	//fill the class
        theDump->fillSim(sim_CTP_vector, point_sim, momentum_sim);
        theDump->fillReco(track_CTP_vector, track_error, point_track, momentum_track);
        //one entry per muon
        theTree->Fill();
	//the rest will not be very useful
...
MuonErrorMatrixAnalyzer::beginJob(const edm::EventSetup& setup)
{
  ...
  edm::LogInfo(theCategory)<<"making a tree named dump for class MuonErrorMatrixDump";
  theTree = new TTree("dump","dump of muon");
  theDump = new MuonErrorMatrixDump();
  theTree->Branch("aDump","MuonErrorMatrixDump",&theDump);
