				       ...
				       DLC& result) const
{
  ...
  if ( !propState.isValid()){ LogDebug("TkNavigation")<<"propagation to barrel failed to check well inside.";return false;}
				       ...
				       DLC& result) const
{
  ...
  if ( !propState.isValid()){LogDebug("TkNavigation")<<"propagation to disk failed to check well inside.";return false;}
  ...
  if ( innerR-deltaR < rpos && rpos < outerR+deltaR) {
    LogDebug("TkNavigation")<<"adding a disk to the reachable layers.";result.push_back( fl);}
  
  if ( innerR+deltaR < rpos && rpos < outerR-deltaR) {
    LogDebug("TkNavigation")<<"return true.";return true;}
  else return false;
}
