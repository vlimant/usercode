...
#aaa MHTU_Path = cms.Path( tpProduction )
#MHTU_Path = cms.Path()
...
#MHTUSchedule = cms.Schedule( )
#remember to extend with the EndPath

def insertMHTU(process):
    process.load('Workspace.MuonHLTTreeUtility.muonHLTTreeUtilityForData_cff')
    process.muonHLTreco = muonHLTrecoSequence(process)
    #  process.MHTU_Path+=process.muonHLTreco
    process.MHTU_Path = cms.Path(process.muonHLTreco )
    process.MHTU_Path.remove(process.hltTriggerType)
    print process.MHTU_Path
    ...
    for p in process.schedule:
        ...
        if (p.__class__==FWCore.ParameterSet.SequenceTypes.EndPath):
            print process.schedule.index(p)
            process.schedule.insert(process.schedule.index(p), process.MHTU_Path )
            break
#    process.schedule.append( process.MHTU_Path )
#    process.thisIsSoFuckingStupid = cms.Path(process.MHTU_Path + process.MHTU_EndPath)
#    print process.thisIsSoFuckingStupid
#    process.schedule.extend( process.MHTU_Path )
